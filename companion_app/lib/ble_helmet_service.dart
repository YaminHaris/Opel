...
/// targets a Raspberry Pi Pico 2 over USB serial, with no BLE GATT server
/// and no contact-storage logic implemented anywhere yet. Coordinate real
/// values with whoever owns the firmware before relying on this in a demo.
class HelmetBleContract {
  static final Guid serviceUuid = Guid('4fafc201-1fb5-459e-8fcc-c5c9c331914b');
  static final Guid emergencyCharUuid = Guid('beb5483e-36e1-4688-b7f5-ea07361b26a8');
  static final Guid ambulanceCharUuid = Guid('beb5483e-36e1-4688-b7f5-ea07361b26a9');

  /// The advertised local name the helmet's microcontroller broadcasts.
  /// Adjust to match whatever firmware actually ships.
...
class BleHelmetService {
  ...
  Future<bool> connect({Duration timeout = const Duration(seconds: 10)}) async {
    ...
    try {
      ...
      if (found == null) {
        lastError = 'No helmet found nearby — make sure it is powered on and in range';
        _statusController.add(BleSyncStatus.error);
        return false;
      }
      ...
      final targetService = services.firstWhere(
        ...
        orElse: () => throw Exception('Helmet did not expose the expected contact service'),
      );
  ...
  Future<bool> writeContacts({
    ...
    try {
      await _emergencyChar!.write(utf8.encode(emergency), withoutResponse: false);
      await _ambulanceChar!.write(utf8.encode(ambulance), withoutResponse: false);
